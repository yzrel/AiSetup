...
import { coerceLoiDocument, printLoiDocument } from "../utils/loiLetter";
...
export function LoiDocumentPreview({
  ...
  const handlePrint = () => {
    printLoiDocument(applicationId);
  };
  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden">
      ...
      <div
        ...
        className="print-a4-sheet p-4 sm:p-8 md:p-10 space-y-5 text-sm text-gray-800 leading-relaxed font-serif bg-white max-h-[70vh] sm:max-h-[520px] overflow-y-auto"
      >
        {/* Enterprise letterhead */}
        <div className="loi-letterhead space-y-0.5">
          <p className="loi-enterprise font-black text-base uppercase tracking-wide">
        ...
        {/* Regional addressee */}
        <div className="loi-block space-y-0.5">
          {(regional.lines?.length ? regional.lines : [regional.name, regional.title])
            .filter(Boolean)
            .map((line, i) => (
              <p key={`regional-${i}`} className={i === 0 ? "loi-first-line font-semibold" : ""}>
                {line}
              </p>
            ))}
        </div>
        ...
        <div className="loi-block space-y-0.5">
          {(thru.lines?.length ? thru.lines : [thru.name, thru.title, thru.officeName])
            .filter(Boolean)
            .map((line, i) => (
              <p key={`thru-${i}`} className={i === 0 ? "loi-first-line font-semibold" : ""}>
                {line}
              </p>
            ))}
        </div>
        ...
        <div className="loi-body space-y-5">
        ...
        <p className="loi-closing">{doc.closing}</p>

        <div className="loi-signature mt-8 space-y-1">
          <p className="loi-typed font-black text-gray-800 border-b border-gray-500 inline-block pr-20 italic pb-1">
            {signature.typedName}
          </p>
        ...
        {(applicationId || doc.generatedAt) && (
          <div className="loi-meta border-t border-gray-200 pt-4 text-xs text-gray-400 space-y-0.5">
